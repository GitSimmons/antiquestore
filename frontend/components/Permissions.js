          ...
          <Table celled structured striped unstackable>
            <Table.Header>
              <Table.Row textAlign="center">
                ...
                <Table.HeaderCell colSpan={possiblePermissions.length}>
                  Permissions
                </Table.HeaderCell>
              ...
              <Table.Row>
                {possiblePermissions.map((possiblePermission) => (
                  <Table.HeaderCell key={possiblePermission}>
                    {possiblePermission}
                  </Table.HeaderCell>
                ))}
              </Table.Row>
...
const UserRow = ({ user }) => {
  ...
  const handleSelect = (e, possiblePermission, user) => {
    if (!userPermissions.includes(possiblePermission)) {
      setUserPermissions((oldPermissions) =>
        oldPermissions.concat(possiblePermission)
      );
    } else {
      setUserPermissions((oldPermissions) =>
        oldPermissions.filter((permission) => permission != possiblePermission)
      );
    }
  ...
  return (
    <Table.Row key={user.id}>
      ...
      <Mutation
        ...
        variables={{ id: user.id, permissions: userPermissions }}
      >
